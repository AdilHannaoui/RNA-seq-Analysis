...
# --------------------------
res_WT_vs_A <- readRDS(file.path(RES_DIR, "res_WT_vs_A.rds"))
res_WT_vs_D <- readRDS(file.path(RES_DIR, "res_WT_vs_D.rds"))
...
perform_enrichGO <- function(res, comparison_name) {
  ...
  enrich_over <- enrichGO(gene = over_genes, OrgDb = org.Sc.sgd.db,
                          ont = GO_ONTOLOGY, pvalueCutoff = PVALUE_CUTOFF)
  
  enrich_under <- enrichGO(gene = under_genes, OrgDb = org.Sc.sgd.db,
                           ont = GO_ONTOLOGY, pvalueCutoff = PVALUE_CUTOFF)
  
  # Save results
  saveRDS(enrich_over, file=file.path(RES_DIR, paste0("enrich_over_", comparison_name, ".rds")))
  saveRDS(enrich_under, file=file.path(RES_DIR, paste0("enrich_under_", comparison_name, ".rds")))
  write.csv(as.data.frame(enrich_over), file.path(RES_DIR, paste0("enrich_over_", comparison_name, ".csv")))
  write.csv(as.data.frame(enrich_under), file.path(RES_DIR, paste0("enrich_under_", comparison_name, ".csv")))
...
# --------------------------
# 5. Optional: plot top GO terms
# --------------------------
grid.arrange(
  dotplot(enrich_WT_vs_A$under, showCategory = 20) + ggtitle("WT vs A - Down"),
  dotplot(enrich_WT_vs_A$over, showCategory = 20) + ggtitle("WT vs A - Up"),
  dotplot(enrich_WT_vs_D$under, showCategory = 20) + ggtitle("WT vs D - Down"),
  dotplot(enrich_WT_vs_D$over, showCategory = 20) + ggtitle("WT vs D - Up"),
  ncol = 2,
  top = textGrob("GO Enrichment Analysis", gp = gpar(fontsize = 16, fontface = "bold"))
)
